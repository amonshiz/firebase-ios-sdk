...
size_t Target::GetSegmentCount() const {
  ...
  for (const Filter& filter : filters_) {
    for (const FieldFilter& sub_filter : filter.GetFlattenedFilters()) {
      ...
      // For instance, it is possible to have an index for "a ARRAY a ASC".
      // Even though these are on the same field,
      // they should be counted as two separate segments in an index.
